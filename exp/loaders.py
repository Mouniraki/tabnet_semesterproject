...
def get_w_vec(df, weights, one_hot=True, sep="_", num_list=[]):
    ...
    for i, c in enumerate(col_names):
        if sep in c and one_hot == True:
            ...
            name = c
        w[i] = weights.get(name, 1000000.0)
    return w

def get_cat_map(df, cat_var, sep="_"):
    col_names = df.columns.values
    c_map = {}
    for i, c in enumerate(col_names):
        if sep in c:
...
class Synthetic(Dataset):
    def __init__(
        ...
        y = df["target"]
        # print(df['AMT_BALANCE'].head())
        # self.cost_orig_df = cost_orig
        X = df.drop(columns=["target"])
        print(X.head())
        random.seed(seed)
        weights = {f: random.choice([0.1, 1.0, 10.0, 100.0]) for f in X.columns}

        X_train, X_test, y_train, y_test = train_test_split(
            X, y, test_size=10000, random_state=seed
        )
        w_vector = get_w_vec(df.drop(columns=["target"]), weights, sep="_").unsqueeze(0)
...
shape_dict = {"ieeecis": 147, "twitter_bot": 19, "home_credit": 183, "syn": 100}
