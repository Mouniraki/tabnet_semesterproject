...
class Attack:
    ...
    def _get_search_args(self):
        ...
        heuristic_funcs = {
            "zero": lambda _: 0,
            "constant": lambda _: 1,
            "random": lambda _: np.random.rand(),
            "linear": (
                lambda x: max(
            ...
            - self.run_params.clf.predict_proba(np.array([x]))[
                0, self.run_params.target_class
            ],
        }

        hash_fn = lambda x: fast_hash(np.array(x.values))
        goal_fn = (
